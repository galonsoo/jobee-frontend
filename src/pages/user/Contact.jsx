...
export default function UserContacts() {
  ...
  useEffect(() => {
    const loadConversations = async () => {
      try {
        const user = getUser();
        const data = await mockApi.getConversations('user', user?.id || 1);
        if (data.success) {
      ...
      <main className="mx-auto w-full max-w-container px-5 py-12 md:px-8 lg:px-12">
        <div className="mb-10">
          ...
          <p className="text-base leading-relaxed text-[#4B5563] mt-3 md:text-lg">
            Mantené contacto directo con empresas.
          </p>
        </div>
                ...
                {conversations.map((conv) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 rounded-full bg-[#FFD65B] flex items-center justify-center text-[#1F2937] font-bold">
                ...
                <>
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {messages.map((msg) => (
                      <div
                        key={msg.id}
                        className={`flex ${msg.senderId.startsWith('user') ? 'justify-end' : 'justify-start'}`}
                      >
                        <div
                          className={`max-w-[70%] rounded-2xl px-4 py-2 ${
                            msg.senderId.startsWith('user')
                              ? 'bg-[#FFD65B] text-[#1F2937]'
                  ...
                  <form onSubmit={handleSendMessage} className="p-4 border-t-2 border-[#FFD65B]">
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="px-4 py-2 rounded-xl bg-[#FFD65B] border-b-4 border-[#E69C00] text-[#1F2937] font-semibold transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
